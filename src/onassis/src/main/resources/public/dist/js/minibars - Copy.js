var minibars = (function() {
  ...
  function init_scales(config) {
    ...
    scale = d3.scale.linear().domain([miny, maxy]).range([datearea_y, center_margin_y])
...
function mouseover(data, i) {
  ...
  if (start && !end && Math.abs(dateDiffInDays(start, data.d)) > max_selection_days) {
    return point.style("cursor", "not-allowed")
  } else if (start && !end) {
    point.style("cursor", "col-resize")
  } else {
    point.style("cursor", "pointer")
  }
}
...
function create(month) {
  ...
  var width = month.columns.length * slot_width
  ...
  minibar_leadin_svg
    .append("line")
    .attr({
    "x1" : leadin_width - line_width,
    "x2" : leadin_width - line_width,
    "y1": datearea_y,
    "y2": bottom_margin_y + 1,
...
function load(new_config) {
  ...
  var bindTo = d3.select('#'+bindto)
  var offsetRect = document.getElementsByClassName("col-md-12")[0].getBoundingClientRect()
  //var new_slot_width = Math.round((bindTo[0].parentNode.clientWidth / (config.months.length*31)))
